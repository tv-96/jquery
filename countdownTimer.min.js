...
$(document).ready(function(){$(".date").each(function(){var a=$(this),b=a.data("start"),c=a.data("gameends"),d=moment(b,"YYYY/MM/DD HH:mm"),e=moment(c,"YYYY/MM/DD HH:mm"),f=moment.utc().format("YYYY/MM/DD HH:mm"),g=d.diff(f,"minutes"),h=e.diff(f,"minutes");switch(!0){case 1<g:var i=moment.utc(b).toDate();i=moment(i).format("YYYY/MM/DD HH:mm"),a.countdowntimer({dateAndTime:i});break;case 2<g:i=moment.utc(b).toDate(),a.html("بعد قليل").addClass("comming-soon");break;case 0<h:a.html("مباشر ").addClass("live"),i=moment.utc(b).toDate();break;default:i=moment.utc(b).toDate(),a.html("إنتهت").addClass("ended");}});});function openTabs(a,b){var c,d,e;for(d=document.getElementsByClassName("tabcontent"),c=0;c<d.length;c++)d[c].style.display="none";for(e=document.getElementsByClassName("tablinks"),c=0;c<e.length;c++)e[c].className=e[c].className.replace(" active","");document.getElementById(b).style.display="flex",a.currentTarget.className+=" active"}
jQuery(function($){moment.tz.setDefault(alba_timezone),$("select#change_timezone").change(function(){current_time_zone=$(this).val(),$.each($(".alba_sports_events-event_item"),function(){var a=$(this).attr("rel");$(this).find(".matchTime").text(moment(a,"YYYY/MM/DD HH:mm").tz(current_time_zone).format(alba_timeformat));$(this).find('.match-data .city').text($('select#change_timezone option:selected').attr('rel'))})})});jQuery(".server-name a").click(function(){jQuery(".server-name a").removeClass("active");jQuery(this).addClass("active")})
